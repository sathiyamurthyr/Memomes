...
import { CheckCircle2, Folder, Eye, Share2, X, Check, Plus, ShieldCheck, Sparkles, FileText } from 'lucide-react';
...
export const UploadSuccessModal: React.FC<UploadSuccessModalProps> = ({
  ...
  const originalName = fileName || meta?.original_file_name || file?.fileNameEncrypted || 'File.png';
  const sizeBytes = fileSize || file?.sizeBytes || meta?.file_size || 0;
  const fileSizeMb = (sizeBytes / (1024 * 1024)).toFixed(2);
  const mimeType = meta?.mime_type || file?.contentTypeEncrypted || 'Document / Asset';
  const dest = destinationPath || destinationDisplay || 'My Files';
      ...
      <div className="w-full max-w-md glass-card rounded-3xl border border-white/10 p-6 space-y-5 shadow-2xl relative text-slate-100 bg-[#080D1A]/95">
        ...
        <div className="p-3 bg-white/[0.02] rounded-xl border border-white/10 flex items-center justify-between text-xs font-mono">
          ...
          <span className="font-bold text-[#F5C027] flex items-center gap-1">
            <Folder className="w-3.5 h-3.5" /> {dest}
          </span>
        </div>
        ...
        <div className="grid grid-cols-5 gap-1.5 text-[10px] font-bold font-mono pt-1">
          <button
            onClick={() => { if (onOpenFolder) onOpenFolder(dest); handleClose(); }}
          ...
          <button
            onClick={() => { if (onViewFile && file) onViewFile(file); else if (onPreview) onPreview(); handleClose(); }}
            className="p-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-slate-200 hover:text-white transition flex flex-col items-center justify-center gap-1"
          >
          ...
          <button
            onClick={() => { if (onShareFile && file) onShareFile(file); else if (onShare) onShare(); handleClose(); }}
          ...
          <button
            onClick={() => { if (onUploadAnother) onUploadAnother(); handleClose(); }}
